...
static void update_counter(RxCheck addr_list[], int index, uint8_t counter) {
  if (index != -1) {
    const CanMsgCheck *msg = &addr_list[index].msg[addr_list[index].status.index];
    const uint8_t counter_step = (msg->counter_step == 0U) ? 1U : msg->counter_step;
    uint8_t expected_counter = (addr_list[index].status.last_counter + counter_step) % (msg->max_counter + 1U);
